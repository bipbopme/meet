...
export function initMatomo({
   ...
   */
  matopush(['setCustomUrl', obfuscateRoomID(location.pathname)]);
...
function obfuscateRoomID(url) {
  return url.replace(/\/r\/(.+)/, (match, id) => {
    const hashedID = sha256(id).slice(0, 10);
    return `/r/hidden-${hashedID}`
  });
}

export function matopush(args) {
  console.log('matopush', args, window._paq);
